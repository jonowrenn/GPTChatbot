...
def build_messages(history, user_msg, topic):
    """Convert Gradio chat history -> OpenAI messages list."""
    messages = [
        {
            "role": "system",
    ...
    # Support both Gradio 4.x (tuples) and Gradio 5.x (dicts) history formats
    for turn in history[-10:]:
        if isinstance(turn, dict):
            # Gradio 5.x: history is a list of {"role": ..., "content": ...} dicts
            messages.append({"role": turn["role"], "content": turn["content"]})
        else:
            # Gradio 4.x: history is a list of [user, assistant] pairs
            u, a = turn
            if u:
